...
for(my $i=2; $i<=$#ARGV; $i++){
    ...
    my $Pho = pearson(\%cHash);#Modify by mosesli2016.11.28
    my ($MAE, $RMSE, $MINinterv, $MAXinterv) = MAE_RMSE(\%cHash);
    my $method = $cHash{"1"}{"method"};

    # Ranking results
    if( $AvgDelta>-$INF ){
	printf "%30s\t:: Ranking: (primary)  DeltaAvg  = %6.3f\t (secondary)  Spearman-Corr = %6.3f\n",
        $method, $AvgDelta, $Rho;
    }
    else{ printf "%30s\t:: Ranking: (primary)  DeltaAvg  = %6s\t (secondary)  Spearman-Corr = %6s\n",
          $method, "--", "--"; }

    # Scoring results
    if( $MAE>-$INF ){#Modify by mosesli2016.11.28
	printf "%30s\t:: Scoring: Pearson r = %6.3f\t (primary) MeanAvgErr = %6.3f\t (secondary) RootMeanSqrErr = %6.3f\t Interval = [%.1f-%.1f]\n",
        $method, $Pho, $MAE, $RMSE, $MINinterv, $MAXinterv;
    }
    else{ printf "%30s\t:: Scoring: (primary) MeanAvgErr = %6s\t (secondary) RootMeanSqrErr = %6s\n",
          $method, "--", "--"; }
}
...
#Modify by mosesli2016.11.28
sub pearson{
	my ($chash) = @_;
	my $n = scalar(keys %{$chash});
	my ($sumxi, $sumyi, $sumxiyi, $sumxi2, $sumyi2) = (0,0,0,0,0);
	for(my $i=1; $i<=$n; $i++){
		my ($xi, $yi) = ($chash->{$i}{"score"}, $chash->{$i}{"refScore"});
		$sumxi += $xi;
		$sumyi += $yi;
		$sumxiyi += $xi * $yi;
		$sumxi2 += $xi ** 2;
	...
	my $pho = ($n * $sumxiyi - $sumxi * $sumyi)/sqrt(($n * $sumxi2 - $sumxi ** 2) * ($n * $sumyi2 - $sumyi ** 2));
	return $pho;
}
