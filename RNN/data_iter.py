...
class TextIterator:
    ...
    @function:文档迭代器
    """
    def __init__(self,source,target,hter,
    ...
    def next(self):
        ...
        if len(self.source_buffer) == 0:
            for k_ in xrange(self.k):
                ...
                if tt == "":
                    break
                ht = self.hter.readline()
                if ht == "":
                    break
                
                self.source_buffer.append(ss.strip().split())
                self.target_buffer.append(tt.strip().split())
                self.hter_buffer.append(ht.strip())
            ...
            if self.sort_by_length:
                ...
                _hbuf = [self.hter_buffer[i] for i in tidx]
                
                self.source_buffer = _sbuf
                self.target_buffer = _tbuf
                self.hter_buffer = _hbuf
            else:
                self.source_buffer.reverse()
                self.target_buffer.reverse()
                self.hter_buffer.reverse()
        ...
        if len(source) == 0 or len(target) == 0:
            source,target,hter = self.next()  
        
        
        return source,target,hter
